    ...
    <span class="back" @click="back()"><img src="../assets/back.png"></span>
      <div class="search-box">
        <form  action="javascript:;">
          <input  type="search" placeholder="搜索" v-model="keyword" @keyup.enter="getResult(keyword)" ref="inputer">
        </form>
      </div>
...
<script>
import {get_search,get_by_seriesId} from '../store/api.js'
export default {
  name: 'SearchRes',
  data () {
    return {
  ...
  beforeRouteEnter:function(to,from,next){
    ...
    let keyword = to.query.key
    next((vm)=>{
      if (type=='search') {
        vm.title='搜索结果'
        get_search(keyword,1,99)
          .then(res=>{
            vm.$store.dispatch('setisRouteLoading',false) 
            vm.pageData = res.result.stories
            if (vm.pageData.length==0) {
              vm.title='暂无关键字的短说'
            }
          })
      }else if(type=='ohtherCha'){
        vm.title = '其他章节'
        get_by_seriesId(keyword)
          .then(res=>{
            vm.$store.dispatch('setisRouteLoading',false)  
            vm.pageData = res.result.stories
          })
  ...
  methods: {
    ...
    getResult: function(key){
      ...
      this.title='搜索结果'
      get_search(this.keyword,1,99)
        .then(res=>{
          this.$store.dispatch('setisRouteLoading',false) 
          this.pageData = res.result.stories
